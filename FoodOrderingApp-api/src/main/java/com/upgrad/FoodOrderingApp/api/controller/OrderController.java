...
// Rest controller for handling Order related endpoint requests
@RestController
public class OrderController {
  ...
  // get coupon details based on coupon name
  @CrossOrigin
  @GetMapping(
  ...
  public ResponseEntity<CouponDetailsResponse> getCouponByCouponName(
      ...
      @PathVariable(name = "coupon_name") final String couponName)
      throws AuthorizationFailedException, CouponNotFoundException {
    String token = getToken(authToken);
    CustomerEntity customerEntity =
        customerBusinessService.checkAuthToken(token, "/order/coupon/{coupon_name}");
    ...
    return new ResponseEntity<CouponDetailsResponse>(
  ...
  public ResponseEntity<CustomerOrderResponse> getPastOrdersOfUsers(
      @RequestHeader(name = "authorization") final String authToken)
    ...
    if (ordersForCustomer == null || ordersForCustomer.isEmpty()) {
      return new ResponseEntity<CustomerOrderResponse>(
          new CustomerOrderResponse().orders(new ArrayList<OrderList>()), HttpStatus.OK);
    }
    ...
    for (OrderEntity e : ordersForCustomer) {
      ...
      OrderListPayment payment =
          new OrderListPayment()
              .id(UUID.fromString(e.getPayment().getUuid()))
              .paymentName(e.getPayment().getPaymentName());
      OrderListCustomer customer =
          new OrderListCustomer()
              .id(UUID.fromString(e.getCustomer().getUuid()))
      ...
      OrderListAddress address =
          new OrderListAddress()
              .id(UUID.fromString(e.getAddress().getUuid()))
              ...
              .state(
                  new OrderListAddressState()
                      .id(UUID.fromString(e.getAddress().getState().getUuid()))
    ...
    return new ResponseEntity<CustomerOrderResponse>(
        new CustomerOrderResponse().orders(responseOrderList), HttpStatus.OK);
  }
  ...
  public ResponseEntity<SaveOrderResponse> saveOrder(
    ...
    if (saveOrderRequest.getCouponId() != null
        && !saveOrderRequest.getCouponId().toString().equals("")) {
    ...
    if (saveOrderRequest.getPaymentId() != null
        && !saveOrderRequest.getPaymentId().toString().equals("")) {
    ...
    if (saveOrderRequest.getRestaurantId() != null
        && !saveOrderRequest.getRestaurantId().toString().equals("")) {
      restaurant =
    ...
    if (saveOrderRequest.getItemQuantities() != null
        && !saveOrderRequest.getItemQuantities().isEmpty()) {
      for (ItemQuantity q : saveOrderRequest.getItemQuantities()) {
    ...
    for (OrderItemEntity e : orderItemObjects) {
      OrderItemEntity orderItemEntityAfterSaving = orderService.saveOrder(e);
    }

    return new ResponseEntity<SaveOrderResponse>(
        new SaveOrderResponse().id(newOrderToBeSaved.getUuid()).status("ORDER SUCCESSFULLY PLACED"),
        HttpStatus.CREATED);
  }
